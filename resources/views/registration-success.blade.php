            ...
            <div class="bg-white rounded-2xl shadow-xl p-8 text-center">
                ...
                <div class="bg-gradient-to-r from-green-50 to-blue-50 rounded-xl p-6 mb-8">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
                        <div>
                            ...
                            <p class="text-lg font-bold text-gray-900">{{ $applicant->registration_number }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600 mb-1">Nama Lengkap</p>
                            <p class="text-lg font-bold text-gray-900">{{ $applicant->applicant_full_name }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600 mb-1">Gelombang</p>
                            <p class="text-lg font-bold text-gray-900">{{ $applicant->wave->wave_name }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600 mb-1">Tanggal Daftar</p>
                            <p class="text-lg font-bold text-gray-900">{{ $applicant->registered_datetime->format('d M Y, H:i') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-6 mb-8 text-left">
                    <div class="flex gap-3">
                        <svg class="h-6 w-6 text-yellow-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
                        </svg>
                        <div>
                            <h3 class="font-semibold text-gray-900 mb-2">Langkah Selanjutnya</h3>
                            <ol class="list-decimal list-inside space-y-2 text-sm text-gray-700">
                                <li>Simpan nomor pendaftaran Anda: <strong>{{ $applicant->registration_number }}</strong></li>
